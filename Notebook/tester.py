...
#
# Automatic tests for Homework#1 in ELTE IK, ANN BSc course part1, 2024 spring
#
...
class Tester:
    ...
    def get_dataset_content(self):
        ...
        if self.dataset_content is None:
            url = "https://nipg12.inf.elte.hu/~vavsaai@nipg.lab/annbsc24_p1/files/student-mat_prep24_noisy.csv"
    ...
    def __test_dataset_load(self, *args):
        ...
        expected_nancount = 155
        expected_nancols = [4,5,8,9]
        expected_nanpos50_60 = [[132, 4], [134, 8], [135, 9], [138, 9], [142, 4], \
                           [143, 5], [144, 4], [148, 8], [152, 5], [153, 5]]
    ...
    def __test_dataset_fill_missing(self, *args):
        ...
        assert np.allclose(np.nansum(self.dataset_noisy), np.sum(dataset[:,:-4]), atol=1e-2), "Tester: Non-NaN values from the 'dataset_noisy' array should be unchanged in the 'dataset' array."

        new_vars = dataset[:,-4:]
        assert np.array_equal(np.unique(new_vars), [0, 1]), "Tester: Incorrect values in new variable columns of the 'dataset' array."
        assert np.allclose(np.sum(new_vars), 1425., atol=1e-2), "Tester: Incorrect values in new variable columns of the 'dataset' array."
    ...
    def __test_dataset_split(self, *args):
        ...
        dataset_split_sizes = {'dataset_split_train': 237, 'dataset_split_val': 79, 'dataset_split_test': 79}
    ...
    def __test_reg_model_iter(self, *args):
        ...
        for dataloader_iter, dataloader_name in zip(dataloader_iters, dataloader_names):
          ...
          assert n_feat == 36, f"Tester: Input feature tensor returned by '{dataloader_name}' have incorrect shape."
    ...
    def __test_reg_model_architecture(self, *args):
        ...
        assert n_params == (36*30 + 30) + (30*20 + 20) + (20*1 + 1), f"Tester: Incorrect structure in 'reg_model' network (incorrect parameter count)."
    ...
    def __test_reg_model_learning(self, *args):
        ...
        test_mse, = args
        assert 1. < test_mse < 5., "Tester: A well trained regression model should provide a test loss (MSE) between 1 and 5."
    ...
    def __test_cl_model_iter(self, *args):
        ...
        for dataloader_iter, dataloader_name in zip(dataloader_iters, dataloader_names):
          ...
          batch_size, n_feat = r0.shape
          assert r1.shape[0] == batch_size, f"Tester: Tensors returned by '{dataloader_name}' have inconsistent shape."
          assert n_feat == 34, f"Tester: Input feature tensor returned by '{dataloader_name}' have incorrect shape."
    ...
    def __test_cl_model_architecture(self, *args):
        ...
        assert n_params == (34*30 + 30) + (30*20 + 20) + (20*3 + 3), f"Tester: Incorrect structure in 'cl_model' network (incorrect parameter count)."
    ...
    def __test_cl_model_learning(self, *args):
        ...
        assert 0.7 < test_ce < 1.3, "Tester: A well trained classification model should provide a test loss (CE) between 0.7 and 1.3"
        assert 0.38 < test_acc < 0.65, "Tester: A well trained classification model should provide a test accuracy between 0.38 and 0.55"
